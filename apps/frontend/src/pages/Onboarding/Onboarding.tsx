...
import { ASSET_OPTIONS, INVESTOR_TYPES, CONTENT_OPTIONS } from './onboarding.constants';
...
export default function Onboarding() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/onboarding', { cryptoAssets, investorType, contentTypes });
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        >
          {isLoading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Generate My Custom Dashboard'}
        </button>
      </form>
    </AuthCard>
  );
